  ...
  for (let i = 0; i < props.lang.length; i++) {
    out.push(
      <div key={i + "graph"} className="graph__lang-div">
        <p>
          {props.lang[i].label} {props.lang[i].value}%
        </p>
...
export default function IndexPage() {
  ...
  function ViewMore(props: { value: boolean }) {
    ...
    if (props.value) {
      return (
        <p onClick={Toggle} className="view-more">
          View Less
        </p>
      );
    }
  ...
  async function GetEvent(user: string) {
    try {
      const data = await Axios.get(
        `https://api.github.com/users/${encodeURIComponent(user)}/events`
      );
  ...
  React.useEffect(() => {
    ...
    if (isEvent && eventData.length > 0) {
      ...
      new Chart(document.querySelector("#hist"), {
        type: "scatter",
        data: {
          labels: toDate,
          datasets: [
  ...
  return (
    <div>
      <section className="info">
        <a href="https://github.com/NikSchaefer/Octo-Stats">
          <Icon.Logo />
        </a>
